...
public class GenerateRegionalOfficeReports {
	private NavBar nav;
...
//	TestCase ID: ????
	@Test(groups="testing")
	public void regionalOfficeLocationsIIREOIRFinal() throws EncryptedDocumentException, IOException {
//		ReadXLSData readData = new ReadXLSData();
//		String[] regionalLocations = readData.getData("Regional Office Locations");
		String[] regionalLocations = {"Central Regional Office", "Eastern Regional Office", "Northern Regional Office"};
		
		for (String loc: regionalLocations) {
				
	//		Set user to Regional Office and Northern
			user.changeUserType(user.regional, loc);
	...
	//		Fill in the appropriate fields in Notification (set location to within the user location, ALGOMA)
			...
			notificationFields.selectLocation(loc + " (RegionalOffice)");
	...
	//		** Store the Incident Report ID
			...
			System.out.println("Created Incident ID for " + loc + ": " + IncidentID);
