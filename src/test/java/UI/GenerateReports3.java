...
//	TestCase ID: ?????
	@Test(groups="testing")
	public void createIncidentReportOnlyEOIR() {
		for(int i = 0; i < 5; i++) {
			nav.createNewReport();
	...
	//		Verify Incident Report is saved
			...
			search.verifyIncident(IncidentID, "EOIR, Death of Staff, Labour Activities"); //level 2 and 3 are not displayed
		}
	}
	
//	TestCase ID: ?????
	@Test(groups="testing")
	public void createIncidentReportOnlyEOIRNotFinal() {
		for(int i = 0; i < 5; i++) {
			nav.createNewReport();
